...
the HFST trimming method. Initially, the HFST language pairs would
also copy dictionaries, but over the last year, we have created
configure scripts that let a language pair have a formal dependency on
one more monolingual data packages. There is now an SVN module
\texttt{languages}\footnote{\href{http://wiki.apertium.org/wiki/Languages}{http://wiki.apertium.org/wiki/Languages}}
where such monolingual data packages reside.
